...
export default function MasterOrderLineTableContr({ errors }: any) {
    ...
    // States
    const tableState = useSelector((state: RootState) => state.masterOrderLines);
    const table = tableState.value;
    const { data: contacts } = useAllResource('contact')
    const numberOfRows = table.length;
    const numberOfColumns = Object.keys(table[0]).length;
    const numberOfBaseColumns = 8;
    const numberOfCustomColumns = numberOfColumns - numberOfBaseColumns;
    const [open, setOpen] = useState<number | null>(null)

    // Computes
    const subtotals = table.map((row) => Number(row.quantity) * Number(row.price_sale));
    ...
    const handlePaste = async (e: React.ClipboardEvent<HTMLTableCellElement>, r: number, c: number) => {
        ...
        for (let i = 0; i < pastedTable.length; i++) {
            for (let j = 0; j < numberOfColumns - c; j++) {
                ...
                if (pastedTable[i][j]) {
                    if (tableState.type[tableState.colNameByIndex[c + j]] === 'number') {
                        pastedValue = pastedTable[i][j].replace(/[^0-9.-]/g, '').replace(/\.(?=.*\.)/, '').replace(/(?!^)-/g, '')
                    }
                    dispatch(setCellValue({ row: r + i, col: c + j, value: pastedValue }))
                }
            }
        }
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (item.kind === "string") {
                item.getAsString((str) => {
                    ...
                    if (imgMatch) {
                        ...
                        dispatch(setCellValue({ row: r + i, col: c, value: base64 }));
                    }
                });
            }
            ...
            if (item.kind === "file") {
                ...
                if (file) {
                    ...
                    reader.onload = () => dispatch(setCellValue({ row: r + i, col: c, value: reader.result }));
                }
            }
        }
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>, r: number, c: number) => {
        ...
        if (files) {
            ...
            const reader = new FileReader();
            reader.readAsDataURL(imageFile);
            reader.onload = () => dispatch(setCellValue({ row: r, col: c, value: reader.result }))
        }
    }


    const textCell = (r: number, c: number, colName: string) => <TableCell key={`${r}${c}`} className="border" onPaste={(e) => handlePaste(e, r, c)}><Input type='text' value={table[r][colName] || ''} onChange={(e) => dispatch(setCellValue({ row: r, col: c, value: e.target.value || '' }))}></Input></TableCell>;
    const prodcutNameCell = (r: number, c: number, colName: string) => <TableCell key={`${r}${c}`} className="border" onPaste={(e) => handlePaste(e, r, c)}><Input type='text' value={table[r][colName] || ''} onChange={(e) => dispatch(setCellValue({ row: r, col: c, value: e.target.value || '' }))} className={cn(`w-[180px]`, errors?.[r]?.product_name?._errors?.[0] && "border-red-500")} /><FieldError>{errors?.[r]?.product_name?._errors?.[0]}</FieldError></TableCell>;
    const descriptionCell = (r: number, c: number, colName: string) => <TableCell key={`${r}${c}`} className="border" onPaste={(e) => handlePaste(e, r, c)}><Input type='text' value={table[r][colName] || ''} onChange={(e) => dispatch(setCellValue({ row: r, col: c, value: e.target.value || '' }))} className={cn(`w-[180px]`, errors?.[r]?.name?._errors?.[0] && "border-red-500")} /><FieldError>{errors?.[r]?.name?._errors?.[0]}</FieldError></TableCell>;
    const numberCell = (r: number, c: number, colName: string) => <TableCell key={`${r}${c}`} className="border" onPaste={(e) => handlePaste(e, r, c)}><Input type='number' value={table[r][colName] === undefined || table[r][colName] === '' ? '' : table[r][colName]} onChange={(e) => dispatch(setCellValue({ row: r, col: c, value: e.target.value || '' }))} /></TableCell>;
    const imageCell = (r: number, c: number, colName: string) => <TableCell key={`${r}${c}`} className="border"><Upload className="w-6 h-6 text-gray-500 absolute cursor-pointer" /><Input type='file' accept='image/*' onPaste={(e) => handlePasteImage(e, r, c)} className='w-6 h-6 float-left absolute cursor-pointer opacity-0' onChange={(e) => handleImageChange(e, r, c)}></Input><img src={table[r][colName] || undefined} className='max-w-20 max-h-20' /></TableCell>;
    const selectCell = (r: number, c: number, colName: string) => (
        <TableCell key={`${r}${c}`} className="border" onPaste={(e) => handlePaste(e, r, c)}>
            <Popover open={open === r} onOpenChange={(isOpen) => setOpen(isOpen ? r : null)}>
                <PopoverTrigger asChild>
                    ...
                    >
                        {table[r][colName]
                            ? contacts && contacts.find((contact: Contact) => String(contact.id) === table[r][colName])?.name
                            : "Select Vendor..."}
                            ...
                            <CommandGroup>
                                {contacts && contacts.map((contact: Contact) => (
                                    <CommandItem
                                        ...
                                        onSelect={() => {
                                            dispatch(setCellValue({ row: r, col: c, value: String(contact.id || '') }))
                                            setOpen(null)
                                        }}
                                    >
                                        <CheckIcon
                                            className={cn(
                                                "mr-2 h-4 w-4",
                                                table[r][colName] === String(contact.id) ? "opacity-100" : "opacity-0"
                                            )}
                                        />
                                        {contact.name}
                                    </CommandItem>
    ...
    const tableCell = (r: number, c: number, colName: string, type: string) => {
        switch (type) {
            case 'binary': return (
                <TableCell key={`${r}${c}`} className="border"><Upload className="w-6 h-6 text-gray-500 absolute cursor-pointer" />
                    <Input type='file' accept='image/*' onPaste={(e) => handlePasteImage(e, r, c)} className='w-6 h-6 float-left absolute cursor-pointer opacity-0' onChange={(e) => handleImageChange(e, r, c)} />
                    <img src={table[r][colName] || undefined} className='max-w-20 max-h-20' />
                </TableCell>
            );
            case 'select': return selectCell(r, c, colName);
            default: return (
                <TableCell key={`${r}${c}`} className="border" onPaste={(e) => handlePaste(e, r, c)}>
                    <Input
                        type={type}
                        value={table[r][colName] || ''}
                        onChange={(e) => dispatch(setCellValue({ row: r, col: c, value: e.target.value || '' }))}
                    // className={cn(`w-[180px]`, errors?.[r]?.product_name?._errors?.[0] && "border-red-500")}
                    />
                    {/* <FieldError>{errors?.[r]?.product_name?._errors?.[0]}</FieldError> */}
                </TableCell>
            );
        }
    };

    const tableRow = (r: number) => (
        <TableRow key={r}>
