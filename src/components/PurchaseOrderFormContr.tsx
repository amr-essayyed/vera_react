...
import { purchaseOrderFormSchema, type tOrderLineCreate, type tOrderLineRead, type tPurchaseOrder, type tPurchaseOrderForm, type tPurchaseOrderRead } from "@/types/purchaseOrder";
...
type Props = {
	purchaseOrder?: tPurchaseOrderRead;
};
...
export default function PurchaseOrderForm({ purchaseOrder }: Props) {
    ...
    // var isOrder = purchaseOrder ? true : false;
    // console.log("isOrder", isOrder, " -> ", purchaseOrder);
    
    var orderId: IdRef = purchaseOrder?.id || null//parseInt(id||'') || null;
	...
	const form = useForm({
		...
		defaultValues: purchaseOrder? {
			...
			partner_id: purchaseOrder.partner_id[0],
			partner_ref: purchaseOrder.partner_ref,
			customer_id: purchaseOrder.customer_id? purchaseOrder.customer_id[0] : undefined,
			date_planned: purchaseOrder.date_planned,
			invoice_status: purchaseOrder.invoice_status,
			// order_line: purchaseOrder.order_line.map((line: tOrderLineRead) => {
			// 	return {
			// 		product_name: Array.isArray(line.product_id)? line.product_id[1]: line.product_id,
    ...
    useEffect(() => {
        if (purchaseOrder) {
            console.log("[on po change]: ", purchaseOrder);
    ...
    const onSubmit = purchaseOrder? onFormSave: onFormSubmit;
